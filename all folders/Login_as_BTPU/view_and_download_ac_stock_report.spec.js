...
test('test', async ({ page }) => {
  ...
  await page.getByRole('combobox').nth(2).selectOption('East');
  await page.getByRole('combobox').nth(4).selectOption('Previous Month');
  ...
  await page.getByRole('combobox').first().selectOption('All');
  await page.getByRole('combobox').nth(4).selectOption('Current Month');
  await page.getByRole('button', { name: 'Submit' }).click();
  await page.waitForTimeout(5000);
  ...
  await page.waitForTimeout(5000);
  await page.locator('div').filter({ hasText: 'AC Stock' }).nth(5).click();
});